...
void splitVolImpl(int min, int max, int SLN, int nDevices, int* startIdx, int* SLNn, float** shvol, float* hvol) {
...
void splitVol(int XN, int YN, int SLN, int nDevices, int* startIdx, int* SLNn, float** shvol, float* hvol) {
    const auto processor_count = std::thread::hardware_concurrency();
    std::vector<std::thread> threads;
    int min = 0;
    int max = XN * YN;
    int step = XN * XN / processor_count;
    int range = 0;
    for(int t = 0; t < processor_count; ++t) {
        if(t == processor_count - 1) {
            range = max;
        } else {
            range = (t+1) * step;
        }
        
        threads.push_back(std::thread(splitVolImpl, min, range, SLN, nDevices, startIdx, SLNn, shvol, hvol));
        min = range + 1;
    }
    for(auto& t : threads) {
        t.join();
...
void gatherProjImpl(int min, int max, int SLN, int nDevices, int* startIdx, int* SLNn, float** shprj, float* hprj) {
  	for (int i = min; i < max; ++i) {
		for (int v = 0; v != SLN; ++v) {
			float val = 0;
			for (int k = 0; k < nDevices; ++k) {
...
void gatherProj(int DNU, int PN, int SLN, int nDevices, int* startIdx, int* SLNn, float** shprj, float* hprj) {
    const auto processor_count = std::thread::hardware_concurrency();
    std::vector<std::thread> threads;
    int min = 0;
    int max = DNU * PN;
    int step = max / processor_count;
    int range = 0;
    for(int t = 0; t < processor_count; ++t) {
        if(t == processor_count - 1) {
            range = max;
        ...
        threads.push_back(std::thread(gatherProjImpl, min, range, SLN, nDevices, startIdx, SLNn, shprj, hprj));
        min = range + 1;
    }
...
void projImpl(int nDevices, float x0, float y0, int DNU, float* xds, float* yds, float imgXCenter, float imgYCenter,
	float** shang, int PN, int XN, int YN, int* SLNn, float** shvol, float** shprj, float dx, byte* mask) {
    std::vector<std::thread> threads;
	for (int i = 0; i < nDevices; ++i) {
        threads.push_back(std::thread(DD2::DD2_gpu_proj_branchless_sat2d, x0, y0, DNU, xds, yds,
			imgXCenter, imgYCenter, shang[i], PN, XN, YN, SLNn[i], shvol[i], shprj[i], dx, mask, i));
	}
    for(auto& t : threads) {
        t.join();
    }
...
void DD2ProjMultiGPUs_CPP17(
    ...
    splitVol(XN, YN, SLN, nDevices, startIdx, SLNn, shvol, hvol);
    
    projImpl(nDevices, x0, y0, DNU, xds, yds, imgXCenter, imgYCenter, shang, 
            PN, XN, YN, SLNn, shvol, shprj, dx, mask);

    gatherProj(DNU, PN, SLN, nDevices, startIdx, SLNn, shprj, hprj);
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    DD2ProjMultiGPUs_CPP17(x0, y0, DNU, xds, yds, imgXCenter, imgYCenter,
