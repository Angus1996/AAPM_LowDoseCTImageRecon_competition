...
void HelicalToFan (
	...
	for(int t = 0; t < processor_count; ++t) {
		...
		threads.push_back(std::thread(HelicalToFanImpl, min, range, Proj, Views, proj, zPos,
			SLN, SD, SO, BVAngle, DetWidth, DetHeight, PerDetW, PerDetH, DefTimes,
			DetCenterW, SpiralPitchFactor, PerDetA, delta, DetCenterHreal, h, deltaZ));
	}
	for(auto& t : threads) {
		t.join();
	}
