...
for ii = 1 : numOfOS
    ...
    rowSum{ii,1} = DD2MutiSlices('Proj',conf_slice,single(ones(2,conf.recon.XN,conf.recon.YN)),Views,view_idx{ii},mask);
    colSum{ii,1} = DD2MutiSlices('Back',conf_slice,single(ones(2,conf.acq.DNU,numOfViews)),Views,view_idx{ii},mask);
...
for ii = 1 : numOfIter
    ...
    for idx = 1 : numOfOS
        ...
        pause(0.0001);    disp(idx);    
        
% %         reconImg(reconImg > 1000) = 0;
    end
