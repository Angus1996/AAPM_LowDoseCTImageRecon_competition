...
void splitProjectionImpl(int min, int max, int SLN, int* SLNn, int nDevices, float** shprj, int* startIdx, float* hprj) {
...
void splitProjection(int SLN, int PN, int DNU, int* SLNn, int nDevices, int* startIdx, float** shprj, float* hprj) {
    ...
    for(int t = 0; t < processor_count; ++t) {
        ...
        threads.push_back(std::thread(splitProjectionImpl, min, range, SLN, SLNn, nDevices, shprj, startIdx, hprj));
        min = range + 1;
    }
    for(auto& t : threads) {
        t.join();
...
void gatherVolImpl(int min, int max, int SLN, int nDevices,  int* SLNn, int* startIdx, float** shvol, float* hvol) {
    for (int i = min; i < max; ++i) {
		for (int v = 0; v != SLN; ++v) {
			float val = 0;
			for (int n = 0; n < nDevices; ++n) {
...
void gatherVol(int XN, int YN, int SLN, int nDevices,  int* SLNn, int* startIdx, float** shvol, float* hvol) {
    const auto processor_count = std::thread::hardware_concurrency();
    std::vector<std::thread> threads;
    int min = 0;
    int max = XN * YN;
    ...
    for(int t = 0; t < processor_count; ++t) {
        ...
        threads.push_back(std::thread(gatherVolImpl, min, range, SLN, nDevices, SLNn, startIdx, shvol,hvol));
    }
    for(auto& t : threads) {
        t.join();
    }
...
void backImpl(int nDevices, float x0, float y0, int DNU, float* xds, float* yds,
	float detCntIdx, float imgXCenter, float imgYCenter,
	float** shang, int PN, int XN, int YN, int* SLNn,
	float** shvol, float** shprj, float dx,	byte* mask) {
    const auto processor_count = std::thread::hardware_concurrency();
    std::vector<std::thread> threads;
    for(int i = 0; i < nDevices; ++i) {
        threads.push_back(std::thread(DD2_gpu_back, x0, y0, DNU, xds, yds, detCntIdx, imgXCenter, imgYCenter, shang[i], PN, XN, YN, SLNn[i], shvol[i], shprj[i], dx, mask, i));
    }
    for(auto& t : threads) {
        t.join();
    }
...
void DD2BackMultiGPUs(
    ...
    splitProjection(SLN, PN, DNU, SLNn, nDevices, startIdx, shprj, hprj);

    backImpl(nDevices, x0, y0, DNU, xds, yds, detCntIdx, imgXCenter, imgYCenter,
        shang, PN, XN, YN, SLNn, shvol, shprj, dx, mask);

    gatherVol(XN, YN, SLN, nDevices, SLNn, startIdx, shvol, hvol);
