...
%
%--------------------------------------------------------------------------
function [Prj, Viws] = HelicalToFan(proj, cfg, zPos)
clc;
...
for iii = 1 : SLN            % set the position of required slice, should be in (h, TotalView * deltaZ - h)
    z = zPos(iii);
    ...
    for i=1:DefTimes
        if mark(i,1)==1
            ...
            PDataNew(id,:)=PData(i,:);
            %markNew(id)=1;
            ViewAnglesNew(id,1)=ViewAngles(i,1);
        end
    end
...
numOfViews = cfg.NumberofSourceAngularSteps; 

Prj = zeros(cfg.NumberofDetectorColumns,numOfViews,SLN);
Viws = zeros(numOfViews,SLN);
for ii = 1 : SLN
    Prj(:,:,ii) = Proj{ii}(1:numOfViews,:)';
    Viws(:,ii) = Views{ii}(1:numOfViews);
    disp(ii);
end
Prj = single(permute(Prj,[3, 1, 2]));
Viws = single(permute(Viws,[2, 1]));
